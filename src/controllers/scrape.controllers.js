...
async function buildWhereClause(titular, anio, numeroExpediente, propiedad) {
  ...
  //Asignar los 0 faltantes
  if(numeroExpediente.length === 1 ){
    numeroExpediente = `000${numeroExpediente}` 
  }
  if(numeroExpediente.length === 2 ){
    numeroExpediente = `00${numeroExpediente}` 
  }
  ...
  console.log(conditions, propiedad);
  if (propiedad) {
    if (anio && !numeroExpediente) {
      console.log("Aqui 1", numeroExpediente);
      return {
        [Op.or]: [
          {
            [Op.and]: conditions,
        ...
        [Op.and]: { asunto: { [Op.like]: `%/${anio}%` } },
      };
    } else if (!anio && numeroExpediente) {
      console.log("Aqui 2");
      return {
        [Op.or]: [
          {
        ...
        [Op.and]: { asunto: { [Op.like]: `%${numeroExpediente}/%` } },
      };
    } else {
      console.log("Aqui 3", `${numeroExpediente}/${anio}`);
      return {
        [Op.or]: [
          {
            [Op.and]: conditions,
        ...
        [Op.and]: { asunto: {[Op.like]: `%${numeroExpediente}/${anio}%`} },
      };
    }
  } else {
    return {
...
async function mapper(req, res) {
  try {
    ...
    let findDb = [];
    const whereClauseParcela = await buildWhereClause(
      titular,
      anio,
      numeroExpediente,
      false
    ...
    const whereClauseActos = await buildWhereClause(
      titular,
      anio,
      numeroExpediente,
      true
    ...
    findDb[0] = await Parcela.findAll({
      where: whereClauseParcela,
    });
    findDb[1] = await Acto.findAll({
      where: whereClauseActos,
    });

    // res.send(result);
    res.status(200).send({ dbResult: findDb[0], scrapingResult: findDb[1] });
  } catch (err) {
