...
m2pc, b = stats.pearsonr(p_df["M2_Construccion"],p_df["Opinion_Valor_Precio_Final"]);
...
nocp, h = stats.pearsonr(p_df["No_Cuartos"],p_df["Opinion_Valor_Precio_Final"]);
nobp, i = stats.pearsonr(p_df["No_banos"],p_df["Opinion_Valor_Precio_Final"]);
p2p, j = stats.pearsonr(p_df["Dos_Plantas"],p_df["Opinion_Valor_Precio_Final"]);
m2cochp, k = stats.pearsonr(p_df["M2_Cochera"],p_df["Opinion_Valor_Precio_Final"]);
ms2tp, l = stats.pearsonr(p_df["minisplit_2ton"],p_df["Opinion_Valor_Precio_Final"]);
ms125tp, m = stats.pearsonr(p_df["minisplit_1_5ton"],p_df["Opinion_Valor_Precio_Final"]);  
ms1tp, n = stats.pearsonr(p_df["minisplit_1ton"],p_df["Opinion_Valor_Precio_Final"]);
norp, o = stats.pearsonr(p_df["No_recamaras"],p_df["Opinion_Valor_Precio_Final"]);
nsp, p = stats.pearsonr(p_df["Nivel_Socioeconomico"],p_df["Opinion_Valor_Precio_Final"]);
...
while r2 < 0.90:
    ...
    var_ind = p_df[["M2", "M2_Construccion", "Ubicacion", "COS", "CUS", "Seguridad","Edad", "No_Cuartos", "No_banos", "Dos_Plantas", "M2_Cochera", "minisplit_2ton", "minisplit_1_5ton", "minisplit_1ton", "No_recamaras", "Nivel_Socioeconomico"]]
    var_dep = p_df["Opinion_Valor_Precio_Final"] 

    # Separación de los datos en conjuntos de entrenamiento y prueba
    X_train, X_test, y_train, y_test = train_test_split(var_ind, var_dep, test_size=0.20)

    # Preprocesamiento de las variables numéricas y categóricas
    num_features = ['M2', 'M2_Construccion',"Edad", 'COS', 'CUS', 'No_Cuartos', 'No_banos', 'M2_Cochera', 'No_recamaras', 'Seguridad', 'minisplit_2ton', 'minisplit_1_5ton', 'minisplit_1ton']
...
#Muestra de Gráficas
#validacion = pd.DataFrame({"Actual": y_test, "Prediccion": y_pred, "Diferencia": y_test-y_pred})
...
num_feature_names = preprocessor.named_transformers_['num'].named_steps['scaler'].get_feature_names_out(num_features)
